...
static void mpu6050_read_raw(int16_t accel[3], int16_t gyro[3], int16_t *temp) {
    ...
    uint8_t val = 0x3B;
    err = i2c_write_blocking(i2c1, addr, &val, 1, true); // true to keep master control of bus
    err = i2c_read_blocking(i2c1, addr, buffer, 6, false);

    for (int i = 0; i < 3; i++) {
        accel[i] = (buffer[i * 2] << 8 | buffer[(i * 2) + 1]);
    }
    // printf("Acc. X = %d, Y = %d, Z = %d\n", accel[0], accel[1], accel[2]);
    // cout << "Acc: " << accel[0] << "|" << accel[1] << "|" << accel[2] << '\n';
    // Now gyro data from reg 0x43 for 6 bytes
    // The register is auto incrementing on each read
    val = 0x43;
    err = i2c_write_blocking(i2c1, addr, &val, 1, true);
    err = i2c_read_blocking(i2c1, addr, buffer, 6, false);  // False - finished with bus
    ...
    val = 0x41;
    err = i2c_write_blocking(i2c1, addr, &val, 1, true);
    err = i2c_read_blocking(i2c1, addr, buffer, 2, false);  // False - finished with bus
