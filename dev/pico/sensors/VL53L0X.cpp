...
#define startTimeout() (timeout_start_ms = time_us_64()/1000)

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && ((uint16_t)(time_us_64()/1000 - timeout_start_ms) > io_timeout))
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  last_status = i2c_write_timeout_us(bus, address, buf, 2, false, 10000);
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  ...
  last_status = i2c_write_timeout_us(bus, address, buf, 3, false, 10000);
  // bus->beginTransmission(address);
  // bus->write(reg);
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  ...
  uint8_t buf[] = {reg, (uint8_t)(value>>24), (uint8_t)(value>>16), (uint8_t)(value>>8), (uint8_t)value};
  last_status = i2c_write_timeout_us(bus, address, buf, 5, false, 10000);
  // bus->beginTransmission(address);
  // bus->write(reg);
...
{
  uint8_t value;
  last_status = i2c_write_timeout_us(bus, address, &reg, 1, false, 10000);
  // bus->beginTransmission(address);
  // bus->write(reg);
  // last_status = bus->endTransmission();

  i2c_read_timeout_us(bus, address, &value, 1, false, 10000);
...
{
  last_status = i2c_write_timeout_us(bus, address, src, count, false, 10000);
  // bus->beginTransmission(address);
  // bus->write(reg);
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  last_status = i2c_read_timeout_us(bus, address, dst, count, false, 10000);
